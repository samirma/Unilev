...
async function main() {
    ...
    try {
        ...
        // Build tokens array dynamically from supported_tokens.json
        const tokens = []
        for (const [symbol, address] of Object.entries(supportedTokens)) {
            if (symbol === "wrapper") continue
            const isWrapper = address.toLowerCase() === wrapperAddress.toLowerCase()
            tokens.push({
                contract: new ethers.Contract(address, erc20Abi, wallet),
                name: symbol,
                address: address,
                needsSwap: !isWrapper,
            })
        ...
        const txApproveHelper = await wrapperContract.approve(
            env.UNISWAPV3HELPER_ADDRESS,
            totalSwapAmount,
            { nonce: nonce++ }
        )
        ...
        for (const token of tokens) {
            ...
            if (token.needsSwap) {
                ...
                if (currentBalance < tokenAmountToDeposit) {
                    console.log(
                        `- Warning: Balance (${ethers.formatUnits(
                            currentBalance,
                            tokenDecimals
                        )}) is slightly less than calculated target. Depositing Max Balance.`
                    )
            ...
            const txApprovePool = await token.contract.approve(poolAddress, tokenAmountToDeposit, {
                nonce: nonce++,
            })
            ...
            const txDeposit = await liquidityPoolContract.deposit(
                tokenAmountToDeposit,
                wallet.address,
                { nonce: nonce++ }
            )
